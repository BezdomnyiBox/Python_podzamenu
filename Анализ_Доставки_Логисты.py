...
def upsert_schedule_entry(supplier, warehouse, weekday, order_deadline, delivery_target):
    """Добавить или обновить запись расписания + зафиксировать историю"""
    db_path = get_db_path()
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()

    # Получаем текущую запись (если есть)
    ...
    # Записываем в историю, только если значение изменилось
    if existing:
...
def get_history(supplier, warehouse):
    """Получить историю изменений для поставщика и склада"""
    conn = sqlite3.connect(get_db_path())
    cur = conn.cursor()
    cur.execute('''
...
def open_edit_schedule_window(supplier=None, warehouse=None, weekday=None):
    ...
    if weekday:
        day_vars[weekday].set(True)
    else:
        for var in day_vars.values():
            var.set(True)  # по умолчанию — все дни
    ...
    def save_schedule():
        ...
        try:
            ...
            messagebox.showerror("Ошибка", "Неверный формат времени (должен быть ЧЧ:ММ)")
            return
    ...
    tk.Button(edit_win, text="Сохранить", command=save_schedule, bg="#2ecc71", fg="white", font=("Segoe UI", 10)).pack(pady=15)
...
def open_schedule_window():
    ...
    # --- ОПРЕДЕЛЯЕМ refresh_schedule_view ДО создания чекбоксов ---
    ...
    for col in cols:
        tree_heading = col
        if col == "Заказ до":
            tree_heading = "Заказ до"
        elif col == "Привоз к":
            tree_heading = "Привоз к"
        schedule_tree.heading(col, text=tree_heading)
    ...
    tk.Button(btn_frame, text="Добавить", command=on_add, bg="#2ecc71", fg="white").pack(side='left', padx=5)
    tk.Button(btn_frame, text="Изменить", command=on_edit, bg="#3498db", fg="white").pack(side='left', padx=5)
    tk.Button(btn_frame, text="История изменений", command=on_history, bg="#9b59b6", fg="white").pack(side='left', padx=5)
    ...
    top.refresh_schedule_view = refresh_schedule_view
    refresh_schedule_view()
...
# ----------------------------
# Основные функции загрузки и анализа
# ----------------------------
def load_file():
...
# ----------------------------
# Ранние привозы — интерфейс с фильтрацией и сортировкой
# ----------------------------
def show_early_deliveries_in_app():
    ...
    if early_df.empty:
        ...
        return
    # Получаем ВСЕ уникальные часы из всего датасета
...
def show_supplier_details(supplier, warehouse):
    ...
    df_subset = df_filtered[mask].copy()
    if df_subset.empty:
        messagebox.showinfo("Информация", "Нет данных для выбранного поставщика и склада.")
        return
    all_hours = sorted(df_current['Час_заказа'].dropna().unique())
    selected_days = [day for day, var in day_filter_vars.items() if var.get()]
    selected_hours = all_hours.copy()
...
# ----------------------------
root = tk.Tk()
root.title("Аналитика доставок версия 4.0")
root.geometry("1200x820")
root.configure(bg="#f5f6fa")

title_label = tk.Label(
    root,
    text="Аналитика доставок версия 4.0",
